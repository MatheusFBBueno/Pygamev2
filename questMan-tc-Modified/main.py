...
def transition_in(inv_peixe,inv_dinheiro,inv_comida,chave_final):
    ...
    while not done:
        ...
        write_fixed(screen,fonte,"DICA: UTILIZE A TECLA 'R' PARA RESETAR E MARCAR SUA POSICAO ATUAL.",WHITE,128,570)
...
def maze(inv_peixe,inv_dinheiro,inv_comida,chave_final):
    background = pygame.image.load("images/FaseLabirindoSaida.png")
    global ponto
    ...
    while not done:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                quit()
        screen.fill(BLACK)
        write_fixed(screen, fonte, "GAME OVER - TIME LIMIT EXCEEDED", WHITE, 100, 200)
        write_fixed(screen, fonte, "JOGO FECHARA AUTOMATICAMENTE AUTOMATICAMENTE EM 3 SEGUNDOS", WHITE, 100, 230)
        pygame.display.flip()
        clock.tick(60)
        time.sleep(3)
        quit()
    ...
    while not done:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if x > 100 and y > 313 and x < 360 and y < 479:
                    quarto(visited_basement, visited_garden)
                elif x > 410 and y > 314 and x < 670 and y < 480:
                    done = True
            if event.type == pygame.QUIT:
                done = True
        screen.blit(background,(0,0))
